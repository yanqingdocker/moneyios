//
//  CGCashToBankCardViewController.m
//  pay
//
//  Created by 胡彦清 on 2018/9/11.
...
@interface CGCashToBankCardViewController ()<UITableViewDataSource,UITableViewDelegate,UITextFieldDelegate,XLPasswordViewDelegate>
...
@implementation CGCashToBankCardViewController
...
- (void)initUI{
    UIView *bankcardView = [[UIView alloc] initWithFrame:CGRectMake(0, 0 , SCREEN_WIDTH, 75)];
    bankcardView.backgroundColor = [UIColor whiteColor];
    [self.view addSubview:bankcardView];
    [bankcardView addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(selectBankCard)]];
    
    _bankcardIcon = [[UIImageView alloc] init];
    [bankcardView addSubview:_bankcardIcon];
    [_bankcardIcon mas_remakeConstraints:^(MASConstraintMaker *make){
        make.left.equalTo(bankcardView).offset(17);
        make.centerY.equalTo(bankcardView);
        make.height.mas_equalTo(@35);
        make.width.mas_equalTo(@35);
    }];
    
    _bankcardTitle = [[UILabel alloc] init];
    _bankcardTitle.textColor = RGBCOLOR(51,51,51);
    _bankcardTitle.font = [UIFont systemFontOfSize:18];
//    _bankcardTitle.backgroundColor = [UIColor redColor];
    [bankcardView addSubview:_bankcardTitle];
    [_bankcardTitle mas_remakeConstraints:^(MASConstraintMaker *make){
        make.left.equalTo(_bankcardIcon.mas_right).offset(14);
        make.top.equalTo(bankcardView.mas_top).offset(20);
        make.height.mas_equalTo(@17);
        make.width.mas_equalTo(@200);
    }];
    
    _bankcardNum = [[UILabel alloc] init];
    _bankcardNum.textColor = RGBCOLOR(102, 102, 102);
    _bankcardNum.font = [UIFont systemFontOfSize:14];
    [bankcardView addSubview:_bankcardNum];
    [_bankcardNum mas_remakeConstraints:^(MASConstraintMaker *make){
        make.left.equalTo(_bankcardIcon.mas_right).offset(14);
        make.top.equalTo(_bankcardTitle.mas_bottom).offset(8);
    ...
    [bankcardView addSubview:rightIcon];
    [rightIcon mas_remakeConstraints:^(MASConstraintMaker *make){
        make.right.equalTo(bankcardView.mas_right).offset(-22);
        make.centerY.equalTo(bankcardView);
        make.height.mas_equalTo(@14);
    ...
    _selectLine = 0;
    _bankcardIcon.image = [UIImage imageNamed:[NSString stringWithFormat:@"%@logo",[[_bankcardArray objectAtIndex:0] objectForKey:@"bankType"]]];
    _bankcardTitle.text = [[_bankcardArray objectAtIndex:0] objectForKey:@"bankType"];
    _bankCard = [[_bankcardArray objectAtIndex:0] objectForKey:@"bankCard"];
    _bankcardNum.text = [NSString stringWithFormat:@"尾号(%@)",[_bankCard substringFromIndex:_bankCard.length- 4]];
    
    UIView *cashView = [[UIView alloc] initWithFrame:CGRectMake(0, 83 , SCREEN_WIDTH, 156)];
    ...
    UIView *accountView = [[UIView alloc] initWithFrame:CGRectMake(0, 245 , SCREEN_WIDTH, SCREEN_HEIGHT - 245-NAVIGATIONBAR_HEIGHT)];//
...
-(void)selectBankCard{
    CGSelectBankCardViewController *vc = [[CGSelectBankCardViewController alloc] init];
    vc.selectLine = _selectLine;
    vc.selectbankcardblock = ^(NSDictionary *selectData,NSInteger selectLine){
//        _bankCard = str;
//        NSLog(@"%@",str);
        _bankcardIcon.image = [UIImage imageNamed:[NSString stringWithFormat:@"%@logo",[selectData objectForKey:@"bankType"]]];
        _bankcardTitle.text = [selectData objectForKey:@"bankType"];
        _bankCard = [selectData objectForKey:@"bankCard"];
        _bankcardNum.text = [NSString stringWithFormat:@"尾号(%@)",[_bankCard substringFromIndex:_bankCard.length- 4]];
